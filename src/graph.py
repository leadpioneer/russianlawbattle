...
Колбэки вывода (стриминг в консоль) замыкаются в узлы при сборке графа:
``on_delta`` — фрагменты генерируемого текста, ``announce`` — объявление
спикера перед генерацией.
"""
...
#: Колбэк объявления спикера: (название роли, номер раунда).
...
def build_graph(
    ...
    *,
    ...
    """Собрать и скомпилировать граф прений; колбэки вывода замыкаются в узлы."""
    ...
    def claimant_turn(state: DebateState) -> dict:
        ...
        if announce:
            announce(SPEAKER_TITLES[ROLE_CLAIMANT], round_number)
    ...
    def defendant_turn(state: DebateState) -> dict:
        ...
        if announce:
            announce(SPEAKER_TITLES[ROLE_DEFENDANT], round_number)
    ...
    def judge_review(state: DebateState) -> dict:
        ...
        round_number = state["round_number"]
        if announce:
            announce(SPEAKER_TITLES[ROLE_JUDGE], round_number)
    ...
    def final_verdict(state: DebateState) -> dict:
        ...
        if announce:
            announce(f"{SPEAKER_TITLES[ROLE_JUDGE]} — итоговое решение", state["round_number"])
...
def run_debate(
    *,
    ...
    :param on_delta: колбэк стриминга текста агентов (вывод в консоль).
    :param announce: колбэк объявления спикера (название роли, номер раунда).
    ...
    graph = build_graph(cfg, on_delta=on_delta, announce=announce)
