...
def render_report(result: DebateResult, generated_at: datetime | None = None) -> str:
    ...
    lines: list[str] = [
        ...
        "",
        "> **Дисклеймер.** Результат является информационно-аналитической симуляцией "
        "(материалом для подготовки к спору), а не юридическим заключением. Он не "
        "заменяет проверку квалифицированным юристом; правовые ссылки требуют сверки "
    ...
    lines += [
        "---",
        "",
        "**Ограничение ответственности.** Данный отчёт — результат информационно-"
        "аналитической симуляции с использованием ИИ. Он не является юридическим "
        "заключением, не создаёт отношений «юрист — доверитель» и не заменяет "
        "проверку квалифицированным практикующим юристом. Все правовые ссылки "
        "перед использованием сверяйте по официальным источникам (pravo.gov.ru).",
        "",
    ]
