...
- **`sonar.py`** — `SonarWebSearchProvider`: веб-поиск через
  `perplexity/sonar-pro-search` на роутере ([OI]-совместимый вызов;
  алиас — `legal_research.search_model` в config.yaml или env `SONAR_MODEL`;
  включение — флаг `legal_research.web_search`, по умолчанию включён). Ищет НПА
  и правоприменительную практику РФ, ссылки фильтруются белым списком
  официальных доменов (pravo.gov.ru, vsrf.ru, sudact и др.). Результаты всегда
  `partially_verified` + warning «сверьте по официальному источнику»;
  usage пишется в `usage_log` с ролью `legal_research` (виден в отчёте).
  Пустой запрос → `[]`; ошибка поиска → `[]` + warning в логах.
