...
Ретраи в `chat()`: транзитные сбои роутера — пустой стрим без контента (в т.ч.
когда приходят только reasoning-дельты; `finish_reason` и объём reasoning пишутся
в лог) и ошибки API до начала генерации (таймаут/обрыв — у `APIError` нет
`status_code`, а также 429/5xx) — повторяются до 2 раз (паузы 2/5 с). Частичный
стрим (контент уже ушёл в `on_delta`/UI) не ретраится — повтор дал бы дубликат
реплики; неповторяемые ошибки (400/401/403) тоже. При исчерпании попыток
`RuntimeError` содержит finish_reason, объём reasoning-дельт, токены и число
попыток.
...
- **`sonar.py`** — `SonarWebSearchProvider`: веб-поиск через плагин `web`
  ...
  по умолчанию включён). Креденшелы и алиас применяются из конфига **сессии**
  (`LegalResearchService.configure(cfg)` перед research): выбор модели поиска
  и роутера в веб-форме переопределяет config.yaml. Ищет НПА и правоприменительную практику РФ. Парсинг
